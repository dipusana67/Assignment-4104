...
\definecolor{PrimaryBlue}{RGB}{25, 85, 150}    % Deep professional blue
\definecolor{AccentTeal}{RGB}{0, 120, 120}     % Modern teal accent
\definecolor{SoftGray}{RGB}{70, 70, 80}        % Readable text
\definecolor{LightMist}{RGB}{245, 248, 250}    % Soft background
\definecolor{CodeBlock}{RGB}{240, 244, 248}    % Code background
\definecolor{AccentOrange}{RGB}{220, 100, 40}  % Warm highlight
...
\hypersetup{colorlinks=true, linkcolor=PrimaryBlue, urlcolor=AccentTeal, citecolor=AccentTeal}

% === RESTRUCTURED SECTION HEADERS ===
\titleformat{\section}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\color{PrimaryBlue!70}\textit{Statistical Data Analysis Report}}
\fancyhead[R]{\color{AccentTeal}\thepage}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\headrule}{\color{AccentTeal!50}\hrule width\headwidth height\headrulewidth}

% TOC Formatting
\renewcommand{\cftsecleader}{\cftdotfill{\cftdotsep}}
\renewcommand{\cftsecfont}{\color{PrimaryBlue}\bfseries}
\renewcommand{\cftsecpagefont}{\color{PrimaryBlue}}
\renewcommand{\cftsubsecfont}{\color{SoftGray}}
...
\lstset{
    language=Python,
    basicstyle=\ttfamily\small\color{SoftGray},
    breaklines=true,
    backgroundcolor=\color{CodeBlock},
    frame=single,
    rulecolor=\color{AccentTeal!30},
    keywordstyle=\color{PrimaryBlue}\bfseries,
    commentstyle=\color{AccentTeal!70}\itshape,
...
\newtcolorbox{outputbox}{
    ...
    title=\textcolor{PrimaryBlue}{\textbf{OUTPUT}}
}
...
\newcommand{\problemtitle}[1]{%
    \vspace{0.5cm}
    \begin{center}
        \colorbox{AccentTeal!15}{\parbox{0.85\textwidth}{\centering
            {\color{PrimaryBlue}\Huge\bfseries Problem \arabic{problemcounter}}\\[0.2cm]
            {\color{SoftGray}\large\bfseries #1}
        }}
    \end{center}
...
\begin{tikzpicture}[remember picture, overlay]
    % Background
    \fill[PrimaryBlue!5] (current page.south west) rectangle (current page.north east);
    
    % Border Frame
    \draw[line width=1.8pt, color=PrimaryBlue] ([xshift=1cm, yshift=-1cm]current page.north west) rectangle ([xshift=-1cm, yshift=1cm]current page.south east);
    \draw[line width=1pt, color=AccentTeal] ([xshift=1.2cm, yshift=-1.2cm]current page.north west) rectangle ([xshift=-1.2cm, yshift=1.2cm]current page.south east);
    
    % Corner decorations
    \foreach \x in {1.2, 1.8} {
        \draw[AccentTeal, line width=0.8pt] ([xshift=\x cm, yshift=-1.2cm]current page.north west) -- ++(0.8,0) -- ++(0,-0.8);
        \draw[AccentTeal, line width=0.8pt] ([xshift=-\x cm, yshift=-1.2cm]current page.north east) -- ++(-0.8,0) -- ++(0,-0.8);
        \draw[AccentTeal, line width=0.8pt] ([xshift=\x cm, yshift=1.2cm]current page.south west) -- ++(0.8,0) -- ++(0,0.8);
        \draw[AccentTeal, line width=0.8pt] ([xshift=-\x cm, yshift=1.2cm]current page.south east) -- ++(-0.8,0) -- ++(0,0.8);
    }

    % University Header
    \node[anchor=north, align=center] at ([yshift=-4cm]current page.north) {
        {\color{PrimaryBlue}\fontsize{22}{22}\selectfont \textbf{BEGUM ROKEYA UNIVERSITY, RANGPUR}}\\[0.4cm]
        {\color{SoftGray}\Large \textsc{Department of Statistics}}\\[1.5cm]
        {\color{AccentTeal}\rule{11cm}{1.2pt}}\\[0.8cm]
        {\color{PrimaryBlue}\fontsize{32}{36}\selectfont \textbf{CATEGOICAL DATA ANALYSIS}}\\[0.5cm]
        {\color{SoftGray}\Large{COURSE CODE :} STAT4104}\\[0.8cm]
        {\color{AccentTeal}\rule{11cm}{1.2pt}}
    };

  % Left Column: Submitted By
\node[anchor=north east, align=left] at ([xshift=-1cm, yshift=-2cm]current page.center) {
    {\large\textbf{\color{PrimaryBlue} SUBMITTED BY}}\\[0.4cm]
    \textbf{\color{SoftGray}Name:} Dipu Sana \\[0.2cm]
    \textbf{\color{SoftGray}ID:} 12110064 \\[0.2cm]
    \textbf{\color{SoftGray}Reg No:} 000015927 \\[0.2cm]
    \textbf{\color{SoftGray}Session:} 2021-2022 \\[0.2cm]
    \textbf{\color{SoftGray}Year/Sem:} 4\textsuperscript{th} Year, 1\textsuperscript{st} Sem
};

% Right Column: Submitted To
\node[anchor=north west, align=left] at ([xshift=1cm, yshift=-2cm]current page.center) {
    {\large\textbf{\color{PrimaryBlue} SUBMITTED TO}}\\[0.4cm]
    \textbf{\color{SoftGray}Name:} Dr. Md. Siddikur Rahman \\[0.2cm]
    \textbf{\color{SoftGray}Title:} Associate Professor \\[0.2cm]
    \textbf{\color{SoftGray}Dept:} Statistics \\[0.2cm]
    \textbf{\color{SoftGray}Inst:} Begum Rokeya University
};

    % Footer Date
    \node[anchor=south] at ([yshift=2cm]current page.south) {
        {\color{SoftGray}\large \textsc{Date of Submission: May 18, 2026}}
    };
\end{tikzpicture}
...
\begin{itemize}
\item Prevalence is defined as the total number of cases divided by the total population:
